defmodule Extream.Seeker do
  def seek(scheduler) do
    ...
    receive do
      {:seek_in_dir, dir, max_depth} ->
        {dirs, paths} = seek_in_dir(dir, max_depth)
  ...
  defp seek_in_dir(%ExtreamSeek.Dir{dir_path: dir_path, depth: depth}, max_depth) do
    case File.ls(dir_path) do
      {:ok, paths} ->
        joined_paths = paths |> Enum.map(&(Path.join([dir_path, &1])))
        is_files = joined_paths |> Enum.filter(&(!File.dir?(&1)))
        is_dirs = joined_paths
                  |> Enum.filter(&(File.dir?(&1)))
                  |> Enum.map(fn (dir_path)-> %ExtreamSeek.Dir{dir_path: dir_path, depth: depth+1} end)
                  |> ExtreamSeek.Dir.dirs_less_than_max(max_depth)
