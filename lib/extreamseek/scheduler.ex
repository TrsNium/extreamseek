...
defmodule ExtreamSeek.Scheduler do
  def run(dirs, process_num, word, max_depth) when max_depth > 0 do
    structed_dirs = dirs
                    |> Enum.map(fn(dir)-> %ExtreamSeek.Dir{dir_path: dir} end)

    1..process_num
    |> Enum.map(fn (_) -> %ExtreamSeek.Process{pid: spawn(Extream.Seeker, :seek, [self()])} end)
    |> schedule_process(structed_dirs, [], word, max_depth)
  end

  defp schedule_process(processes, dirs, paths, word, max_depth, results \\ []) do
    receive do
      {:ready, pid} when dirs != [] ->
        ...
        schedule_process updated_processes, other_dirs, paths, word, max_depth, results
      {:ready, pid} when paths != [] ->
        {target_path, other_paths} = List.pop_at(paths, 0)
        updated_processes = ExtreamSeek.Process.update_process(pid, processes, :execute)
        send pid, {:seek_in_file, target_path, word}
        schedule_process updated_processes, dirs, other_paths, word, max_depth, results
      {:ready, pid} ->
        case ExtreamSeek.Process.is_all_completed(processes) do
          true ->
            ...
            case deleted_processes do
              [] -> results
              _  -> schedule_process deleted_processes, dirs, paths, word, max_depth, results
            end
          false ->
            send pid, :idle
            schedule_process processes, dirs, paths, word, max_depth, results
        end
      ...
      {:completed_seek_in_dir, pid, new_dirs, new_paths} ->
        ...
        schedule_process updated_processes, updated_dirs, updated_paths, word, max_depth, results

      # Handler when the directory has been scanned.
      {:completed_seek_in_file, pid, file} ->
        updated_processes = ExtreamSeek.Process.update_process(pid, processes, :completed)
        case file.is_contain do
          true -> schedule_process updated_processes, dirs, paths, word, max_depth, results ++ [file]
          false -> schedule_process updated_processes, dirs, paths, word, max_depth, results
        end
    end
  end
end
